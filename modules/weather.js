...
function getWeatherLocation(words, channel, nick, cb) {
  ...
  let inserter = () => {
    db.query("SELECT id FROM weather WHERE channel = $1::text AND nick = $2::text", [channel, nick], (err, res) => {
      if ( err ) {
      ...
      let args = null;
      if (entries === 0) {
        query = "INSERT INTO weather (channel, nick, location) VALUES ($1, $2, $3)";
        args = [channel, nick, location];
      } else {
        query = "UPDATE weather SET location = $1 WHERE channel = $2::text AND nick = $3::text";
  ...
  if (location.trim() === "") {
    ...
    db.query("SELECT location FROM weather WHERE nick = $1::text AND channel = $2::text", [nick, channel], (err, res) => {
