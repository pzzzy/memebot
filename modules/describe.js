...
function exec(bot, words, from, to) {
  ...
  let url = `http://api.duckduckgo.com/?q=${escape(query)}&format=json&pretty=1&no_html=1&t=rememe&skip_disambig=1`;
  const sendTo = to == _bot.nick ? from : to;

  request(url, (err, response) => {
    const resp = JSON.parse(response.body);
    if (resp["AbstractText"] != "" && resp["AbstractURL"] != "") {
      let text = resp["AbstractText"].replace(/\n/g, " ").slice(0, 400 - resp["AbstractURL"].length);
